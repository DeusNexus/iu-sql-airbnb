...
for i, user in enumerate(admin_users, start=1):
    ...
    line2=f"VALUES (NULL, 1, NULL, '{user[0]}', '{user[1]}', '123-456-789{i}', '{user[2]}', '{user[3]}', current_date - random() * INTERVAL '365 days', 'https://www.linkedout.com/{user[0]}-{user[1]}-{i}', '{user[4]}');"
    print(line1)
    print(lines)
    lines.append(line1)
    lines.append(line2)
    users.append({
        ...
        "social_link": 'https://www.linkedout.com/{user[0]}-{user[1]}-{i}',
        "about_me": '{user[4]}',
    })


with open('insert-user-admins.sql','a') as sql:
    for line in lines:
        sql.write(line+'\n')

with open('insert-user-admins.json','w') as file:
